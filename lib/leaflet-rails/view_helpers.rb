      ...
      if markers
        markers.each_with_index do |marker, index|
          if marker[:icon]
            ...
            output << "var #{icon_settings[:name]}#{index} = L.divIcon({iconSize: #{icon_settings[:icon_size]}, iconAnchor: #{icon_settings[:icon_anchor]}, popupAnchor: #{icon_settings[:popup_anchor]}, className: #{icon_settings[:class_name]}, html: #{icon_settings[:html]});"
            output << "marker = L.marker([#{marker[:latlng][0]}, #{marker[:latlng][1]}], {icon: #{icon_settings[:name]}#{index}}).addTo(map);"
          else
